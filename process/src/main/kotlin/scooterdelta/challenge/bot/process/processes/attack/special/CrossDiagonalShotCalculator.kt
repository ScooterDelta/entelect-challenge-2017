...
class CrossDiagonalShotCalculator : AbstractProbabilityCalculator() {
    ...
    override fun getTargetCells(cell: OpponentCell, map: Map<OpponentCell>): List<OpponentCell> {
        ...
        for (y in cell.y - 1..cell.y + 1) {
            (cell.x - 1..cell.x + 1)
                    ...
                    .filter { it.x != cell.x && it.y != cell.y || it.x == cell.x && it.y == cell.y }
                    .filter { !it.damaged }
                    .filter { !it.missed }
    ...
    override fun getCode(): Code {
        return Code.FIRE_CROSS_SHOT_DIAGONAL
    }
}